...
const FieldLabel: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <Text style={{ fontSize: 12.5, fontWeight: "700", opacity: 0.9, color: colors.TEXT }}>
      {children}
    </Text>
  );
};
...
const SimpleSelect: React.FC<{
  ...
  return (
    <>
      ...
      >
        <Text style={{ flex: 1, color: selectedLabel ? colors.TEXT : colors.PLACEHOLDER }}>
          {selectedLabel ?? placeholder}
        </Text>
        {rightAdornment ?? <MaterialIcons name="expand-more" size={20} color={colors.TEXT} />}
      </Pressable>
            ...
            <ScrollView style={{ maxHeight: 220 }}>
              {options.map((opt) => (
                ...
                >
                  ...
                  {value === opt.value && <Ionicons name="checkmark" size={18} color={colors.TEXT} />}
                </Pressable>
              ))}
            </ScrollView>
          </View>
...
export default function UpdateManifest() {
  ...
  const manifestId = Array.isArray(id) ? id[0] : id; 
  const { colors, isDark } = useTheme();
    ...
    const fetchData = async () => {
      ...
      try {
        const [trackerRes, placeRes, userRes, manifestRes] = await Promise.all([
          ...
          fetch(`${API_BASE_URL}/v1/manifests/${manifestId}`, { headers }),
        ]);
        ...
        setTrackerOptions(
          (Array.isArray(trackers) ? trackers : []).map((t: any) => ({
            label: t.name ?? `Tracker #${t.id}`,
            value: String(t.id),
          }))
        );
        setPlaceOptions(
  ...
  const updateManifest = async () => {
    try {
      const token = await AsyncStorage.getItem("access_token");
      const orgId = await AsyncStorage.getItem("organisationId");
      if (!token || !orgId || !manifestId) throw new Error("Missing credentials or manifest ID");
      ...
      Alert.alert("Success", `Manifest ${manifestId} updated.`);
    ...
    return (
      <SafeAreaView style={s.safe}>
        ...
        <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
          ...
          <Text style={{ marginTop: 12, color: colors.MUTED }}>Loading manifest data...</Text>
        </View>
      </SafeAreaView>
    );
  }
              ...
              <>
                ...
                <View style={s.fieldRow}>
                  ...
                  <Pressable onPress={() => setShowEndPicker(true)} style={s.input}>
                    <View style={{ flexDirection: "row", alignItems: "center", flex: 1 }}>
                      <Text style={{ flex: 1, color: endTime ? colors.TEXT : colors.PLACEHOLDER }}>
...
const styles = ({ colors, isDark }: { colors: any; isDark: boolean }) => {
  const base = baseStyles({ colors, isDark });
